        ...
        if chat_type == "private":
            ...
            if not user_exists or (user and user.get("status") == "pending"):
                if contact:
                    ...
                    send_telegram_message(chat_id, "👋 Welcome to Naifaru Blood Donors.\nPlease click the START button below to proceed.\n\n                                        👇👇👇", reply_markup=keyboard)
...
async def update_password(request: Request):
    ...
    try:
        ...
        res = supabase.table("admin_users").update({"password": new_pass}).or_(f"phone_number.eq.{username},username.eq.{username}").execute()
        return {"status": "ok"}
    except Exception as e:
...
async def create_user(request: Request):
    ...
    try:
        ...
        # Generate Fake ID (negative timestamp to avoid collision with Telegram IDs)
        import time
